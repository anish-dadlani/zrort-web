...
class LoginModel extends Model
{
	...
	public function authenticate_user($username, $password)
	{
		...
		if (count($row) > 0)
		{
			$row['pk_id'] = $this->db->table('business_admins_login_log')->insert($data);
			return $row;
		}elseif(count($row) ==0)
		{
			$data['login_attempt_result'] = 'Neglect';
			$this->db->table('business_admins_login_log')->insert($data);
			return 0;
		}
		else {
			return 0;
	...
	public function  update_authenticate_user($businessuser_id){
		$data = array(
			'logout_time'=> date('Y-m-d H:i:s')
		);
		$query =$this->db->table('business_admins_login_log')->where(['admin_id' => $businessuser_id])->update($data);
	} 
